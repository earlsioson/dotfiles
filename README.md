...
`--prerelease allow` is required while Mojo is in beta. Avoid `uv tool install mojo`: tool isolation breaks Mojo's cross-package binary dependencies and the compiler fails to locate LLDB. Because uv installs the toolchain into the project's `.venv` rather than onto `$PATH`, `es/mojo.lua` looks for `.venv/bin/mojo` beneath the nearest `pyproject.toml` before falling back to `$PATH` — the same resolution order Pyright uses to find a project interpreter. That binary is what proves the toolchain exists, but the runner then invokes `uv run --no-sync mojo` from the project root rather than the path it found. Running `.venv/bin/mojo` directly inherits whatever environment launched Neovim, and mojo cannot locate the companion binaries shipped beside it under site-packages — `crashpad_handler` among them — unless something in that environment points at the virtualenv, so the same keypress behaves differently depending on whether the launching shell happened to be activated. `uv run` supplies that environment itself, which is also why no activation step is needed. The language server is resolved the same way by `es/lsp/mojo.lua`, `.venv/bin/mojo-lsp-server` before `$PATH`.
